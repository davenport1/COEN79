...
#pragma mark - Toolkit Function Implementation
    ...
    // to a copy of the removed item.
    {
        ...
        else //next right node is NULL
        {
            //now at end of subtree
            removed = root_ptr->data(); //assign original data to this nodes data 
            oldroot_ptr = root_ptr;
            root_ptr = root_ptr->left();    
            delete oldroot_ptr;         //clear heap mem
        }
    }
...
#pragma mark - Member Function Implementation
    ...
    // Header file used: bintree.h
    {
        ...
        do
        {
            ...
            {   // Go left
                if(cursor->left() == NULL) {
                    binary_tree_node<Item> *temp = new binary_tree_node<Item>(entry);   //create new node with entry
                    cursor->set_left(temp);                                             //set left link from cursor
                    done = true;                                      
                }
                else
                    cursor = cursor->left();
            ...
            {   // Go right
                if(cursor->right() == NULL) {
                    binary_tree_node<Item> *temp = new binary_tree_node<Item>(entry);   //create new node with entry
                    cursor->set_right(temp);                                            //set right link from cursor
                    done = true;
                }
                else
                    cursor = cursor->right();
    ...
    // method.
    {
        if (addroot_ptr != NULL)
        {
            //insert each item from the addroot_ptr binary tree into the bag that activated this in a pre-order traversal

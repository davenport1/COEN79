...
namespace coen79_lab8
{
    ...
    void deque<Item>::operator=(const deque<Item>& source) {
        ...
        for (size_type bp_array_index = 0; bp_array_index < source.bp_array_size; ++bp_array_index)
        {
            ...
            else
            {
                ...
                if(&(source.block_pointers[bp_array_index]) == source.first_bp)         //if the current block_pointers address matches the first_bp in source
                    first_bp = &(block_pointers[bp_array_index]);
                
                
                //If this is the last_bp of source, then set the last_bp of this deque
                if(&(source.block_pointers[bp_array_index]) == source.last_bp)  //if the current block_pointers address matches the last_bp in source
                ...
                {
                    //sets the data of the current datablock index equal to the same data at the same index in the deque being copied
                    (block_pointers[bp_array_index])[block_item_index] = (source.block_pointers[bp_array_index])[block_item_index];
                    if(source.front_ptr == &(source.block_pointers[bp_array_index])[block_item_index])    //determine if current item inserted into the deque was
                        front_ptr = &(block_pointers[bp_array_index])[block_item_index];                    // the item in source pointed to by either the front or back ptr
    ...
    void deque<Item>::clear () {
        ...
        for (size_type bp_array_index = (first_bp - block_pointers); bp_array_index < diff; ++bp_array_index) {
            if(block_pointers[bp_array_index] != NULL) {                //if this block_ptr is initialized with a data block
                delete []block_pointers[bp_array_index];                //delete the data block
                block_pointers[bp_array_index] = NULL;
            }
        }
    ...
    void deque<Item>::pop_front()
    {
        ...
        else if (front_ptr == ((*first_bp) + block_size - 1))
        {
            ...
            ++first_bp;                                         //move first_bp ptr up 1
            front_ptr = *first_bp;                              //set front ptr to beginning of data block
        }

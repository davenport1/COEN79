...
//
//   void list_delete_item (node* head_ptr, std::string& name)
//      Pre Condition: head_ptr is the head pointer of a linked list, name is a non-empty string,
//      and there exists a node within the list conatining the item
//      Post Condition: heap memory returned, node containing name destroyed, and link established 
//      between the node before and after the node that was deleted
//
...
namespace coen79_lab7
{
    ...
    node* list_search(node* head_ptr, const std::string& name);
    const node* list_search (const node* head_ptr, const std::string& name);
    void list_delete_item (node* head_ptr, const std::string& name);
}

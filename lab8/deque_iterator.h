...
namespace coen79_lab8
{
    ...
    public:
        ...
        deque_iterator& operator ++( ) // Prefix ++
        {
            ...
            // Set the variables. Return the itertaor.
            if (cursor == back_ptr)
            {
                cursor = NULL;
            ...
            {
                current_block_pointer = (current_block_pointer + 1);                //advance te current block pointer once
                current_boundary = *current_block_pointer + (block_size - 1);       //assign the boundary as the last element of the data block the current 
                cursor = *current_block_pointer;                                            //block pointer points to
                                                                                    //set cursor to the beginning of the next data block
            }
            // Move forward
            else
            {
                cursor = (cursor + 1);                                              //advance the cursor                                         
            }
